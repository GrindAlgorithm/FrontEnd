...
- 현재 프론트는 **목(mock) 모드**로 완전 동작 중. 백엔드가 본 문서대로 구현되면
  `.env`의 `VITE_USE_MOCK=false` 한 줄로 실서버 연동 전환
...
2. **전환 절차**: `.env.development`에서 `VITE_USE_MOCK=false` → `npm run dev` → Vite 프록시 경유로 `:8080` 호출. 화면별로 켜고 끄는 옵션은 없음(전역 스위치)
